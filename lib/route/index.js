    ...
    return function () {
      ...
      this.getConfig()
        .then(requestedConfig => {
          ...
          return utils.findModuleFile(wipPath);
        })
        .then(foundModulePath => {
          modulePath = foundModulePath;
          return pify(fs.readFile)(modulePath, 'utf8');
        })
        // if file doesn't have the dependency, add it
        .then(moduleFile => pify(fs.writeFile)(modulePath, ngAddDep(moduleFile, dependency)))
        .then(() => {
          // get name-routes.{coffee,js,ts}
          return utils.findRoutesFile(wipPath);
        })
        .then(foundRoutesPath => {
          routesPath = foundRoutesPath;
          return pify(fs.readFile)(routesPath, 'utf8');
        })
        // add route to route file
        .then(routesFile => pify(fs.writeFile)(routesPath, utils.addRoute(routesFile, newState, newRouteConfig)))
        .then(() => {
          this.copyE2e(config);
