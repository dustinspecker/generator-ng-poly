...
module.exports = yeoman.Base.extend({
    ...
    this.prompt([
      {
        ...
        async choices() {
          const choicesDone = this.async()
            , directory = await utils.getAppDir()
            , files = await pify(recursiveReaddir)(directory);

          choicesDone(await utils.moduleFilter(files));
        }
      },
      {
        name: 'url',
  ...
  async getConfig() {
    ...
    modules = await utils.extractModuleNames(this.module);
