...
export default class GenBase extends Base {
    ...
    this.prompt([
      {
        ...
        choices() {
          ...
          utils.getAppDir().then(appDir => pify(recursiveReaddir)(appDir))
            .then(files => utils.moduleFilter(files))
            .then(modules => moduleDone(modules));
        }
      },
      {
        name: 'url',
      ...
      },
      {
        ...
        default(answers) {
          const module = answers.module || options.module
            , moduleDone = this.async();

          utils.normalizeModulePath(module).then(normalizeModulePath => {
            moduleDone(normalizeModulePath + '/' +
  ...
  getConfig() {
    ...
    return utils.getAppDir().then(appDir => {
      config.appDir = appDir;
      return utils.getAppName();
    }).then(appName => {
      config.appName = appName;
      return utils.getUnitTestDir();
    }).then(testDir => {
      ...
      return utils.extractModuleNames(this.module)
        .then(modules => {
          config.moduleName = utils.lowerCamel(modules[0]);
          config.parentModuleName = utils.lowerCamel(modules[1]);
          return utils.normalizeModulePath(this.module);
        }).then(normalizeModulePath => {
          config.modulePath = normalizeModulePath;
