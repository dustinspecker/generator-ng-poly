...
function hasParam(fileContents, config) {
  ...
  let opts = config.appScript === 'ts' ? {language: 'ts'} : null;
...
function addState(lines, state, analysis, config) {
  ...
  lineToCheck = lines[analysis.existingRouteFound ? analysis.routeStartIndex : analysis.configFunctionIndex];
 ...
 */
exports.addRoute = function (fileContents, state, config) {
