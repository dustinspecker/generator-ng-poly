 ...
 * @param {Object} [opts] - options to pass to module filter
 * @param {Boolean} [opts.polymer] - is Polymer being used?
 * @param {Boolean} [opts.type] - is module-type structure being used?
 * @return {Object[]} - list of modules
 */
exports.moduleFilter = function moduleFilter(files, opts) {
  ...
  // not currently passing polymer opt, so always run
  if (!opts.polymer) {
    filteredFiles = filteredFiles.filter((file) => {
      return file.indexOf('components') === -1;
    });
  }

  // not currently passing type opt, so always run
  if (!opts.type) {
    filteredFiles = filteredFiles.filter((file) => {
