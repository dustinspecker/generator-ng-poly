...
function convertToChoice(module) {
  return appUtils.getAppDir().then(appDir => ({
    name: module,
...
function findFile(filePath, isLookingForRoutes) {
  ...
  return Promise.resolve(_.find(files, routesFile => fs.existsSync(routesFile)));
}
...
export default {
  ...
  extractModuleNames(modulePath) {
    return appUtils.getAppDir().then(appDir => {
      ...
      if (modulePath === appDir) {
        return appUtils.getFileFromRoot('package.json').then(pkg => [pkg.name, null]);
      }
