...
module.exports = genBase.extend({
  ...
  async writing() {
    let filePathToCheck, parentDir, parentModuleName;

    this.context = await this.getConfig();

    // extract the new module name (last part of path)
    ...
    this.context.modulePath = await utils.normalizeModulePath(this.module);

    // create new module directory
    await pify(mkdirp)(path.join(this.context.appDir, this.context.modulePath));
    ...
    // get module path
    const filePath = await utils.findModuleFile(filePathToCheck);

    // if adding dep to app file, then dep is `module
    // else dep is `parent.module`
    const depName = (this.context.parentModuleName ? this.context.parentModuleName + '.' : '') + this.context.lowerCamel;

    // add dep and save modifications
    await pify(fs.writeFile)(filePath, ngAddDep(await pify(fs.readFile)(filePath, 'utf8'), depName));

    // create new module
    const moduleDest = path.join(this.context.appDir, this.context.modulePath, this.context.hyphenModule);
    await this.copySrcFile('module', `${moduleDest}-module.${this.context.appScript}`, this.context);
    if (this.options && !this.options.empty) {
      await this.copySrcFile('module-routes', `${moduleDest}-routes.${this.context.appScript}`, this.context);
    }
  },
